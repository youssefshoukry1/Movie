...
export default function Favourites() {
    ...
    const { data: FavouritesData, isLoading } = useQuery({
        queryKey: ["getMoviesFavourites", page],
        queryFn: async () => {
            const { data } = await axios.get(
                `${environment.apiBaseUrl}account/${account}/favorite/movies?api_key=${environment.api_Key}&session_id=${Signup}&language=en-US&sort_by=created_at.asc&page=${page}`,
                {
                    headers: {
    ...
    return (
        ...
        mt-4 lg:mx-20 sticky top-4 z-50 animate-fadeIn my-10"
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-8 lg:gap-6 gap-5 mb-12">
                {FavouritesData?.map((movie2, index) => (
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1)
                    ...
                    .map((num) => (
                        <button
                            key={num}
                            onClick={() => setPage(num)}
                            className={`px-3 py-1 rounded-lg transition-all ${page === num
